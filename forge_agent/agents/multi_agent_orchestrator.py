"""
forge_agent/agents/multi_agent_orchestrator.py
...
# Example:
#   t001 (ME)           → depth 0  ─┐
#   t003 (Materials)    → depth 0  ─┤ asyncio.gather(t001, t003)
#   t002 (Controls)     → depth 1  ─┘ (waits for t001)
#   t004 (Safety)       → depth 2      (waits for t002)
...
def find_independent_groups(
    ...
    Raises ValueError on cyclic dependencies (should never happen from a
    well-prompted Orchestrator, but must be caught defensively).
    """
    # Build id → task map
    ...
    while current_wave:
        # Sort within wave by priority (lower = earlier)
    ...
    # Cycle detection: if any tasks remain with in_degree > 0, there's a cycle
...
class MultiAgentOrchestrator:
    ...
    async def solve(
        ...
        for group in groups:
            # All tasks in this group are independent — run concurrently
            group_coroutines = [
        ...
        for disagreement in self.blackboard.disagreements:
            if disagreement["resolution"] is None:
                ...
                if isinstance(resolution, HumanEscalation):
                    # Stamp as requires-human-review, not resolved
