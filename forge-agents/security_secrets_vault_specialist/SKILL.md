# Security Secrets Vault Specialist — SKILL Definition

**Agent ID:** `security_secrets_vault_specialist`
**Domain:** Security & Secrets Vault — Secrets Management, Auth, Vault, RBAC, Zero-Trust
...
The Security Secrets Vault Specialist performs security & secrets vault analysis, synthesising results
...
|---|---|---|
| Core security & secrets vault analysis (analytical) | Active (MVP) | Level 1 capability |
| Security & Secrets Vault design sizing | Active (Level 2) | Parametric methods |
| Sensitivity analysis | Planned (V1) | Level 3 |
| Numerical security & secrets vault methods | Planned (V1) | Level 3 |
| Multi-physics security & secrets vault coupling | Planned (V2) | Level 4 |
| Uncertainty quantification | Planned (V2) | Level 4 |
| Full-chain security & secrets vault optimisation | Planned (V3) | Level 5 Master |
...
Every Security Secrets Vault Specialist output MUST include:
...
6. `security_&_secrets_vault_summary` — structured domain summary (see Output Contract)
...
confidence: 0.75
security_&_secrets_vault_summary:
  method: "analytical"
  primary_result: 0.0
  units: "[specify]"
  safety_margin: 0.0
...
Raise **[SECURITY & SECRETS VAULT SIMULATION REQUIRED]** when:
...
|---|---|---|---|
| 1 | Novice | 0.00–0.39 | Core security & secrets vault analysis — analytical methods |
...
- **Regime violation**: applying security & secrets vault method outside its stated validity range without checking boundary conditions
...
paper_section_draft:
  ...
  subsection_title: "Security & Secrets Vault Analysis"
  content_latex: "..."
  equations_numbered: true
  notation_consistency: true

peer_review_verdict:
  target_agent_id: "security_secrets_vault_specialist"
  target_run_id: "..."
...
- Domain-specific standards and handbooks for security & secrets vault
