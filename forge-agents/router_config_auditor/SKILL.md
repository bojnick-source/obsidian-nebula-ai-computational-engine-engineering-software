# Router Config Auditor — SKILL Definition

**Agent ID:** `router_config_auditor`
**Domain:** Router Config Auditor — Task routing rules, agent selection logic, config drift
...
The Router Config Auditor performs router config auditor analysis, synthesising results
...
|---|---|---|
| Core router config auditor analysis (analytical) | Active (MVP) | Level 1 capability |
| Router Config Auditor design sizing | Active (Level 2) | Parametric methods |
| Sensitivity analysis | Planned (V1) | Level 3 |
| Numerical router config auditor methods | Planned (V1) | Level 3 |
| Multi-physics router config auditor coupling | Planned (V2) | Level 4 |
| Uncertainty quantification | Planned (V2) | Level 4 |
| Full-chain router config auditor optimisation | Planned (V3) | Level 5 Master |
...
Every Router Config Auditor output MUST include:
...
6. `router_config_auditor_summary` — structured domain summary (see Output Contract)
...
Raise **[ROUTER CONFIG AUDITOR SIMULATION REQUIRED]** when:
...
|---|---|---|---|
| 1 | Novice | 0.00–0.39 | Core router config auditor analysis — analytical methods |
...
- **Regime violation**: applying router config auditor method outside its stated validity range without checking boundary conditions
...
paper_section_draft:
  ...
  subsection_title: "Router Config Auditor Analysis"
  content_latex: "..."
  equations_numbered: true
  notation_consistency: true

peer_review_verdict:
  target_agent_id: "router_config_auditor"
  target_run_id: "..."
...
- Domain-specific standards and handbooks for router config auditor
