# Degraded Mode Monitor — SKILL Definition

**Agent ID:** `degraded_mode_monitor`
**Domain:** Degraded Mode Monitor — Agent fallback activation, capability reduction tracking
...
The Degraded Mode Monitor performs degraded mode monitor analysis, synthesising results
...
|---|---|---|
| Core degraded mode monitor analysis (analytical) | Active (MVP) | Level 1 capability |
| Degraded Mode Monitor design sizing | Active (Level 2) | Parametric methods |
| Sensitivity analysis | Planned (V1) | Level 3 |
| Numerical degraded mode monitor methods | Planned (V1) | Level 3 |
| Multi-physics degraded mode monitor coupling | Planned (V2) | Level 4 |
| Uncertainty quantification | Planned (V2) | Level 4 |
| Full-chain degraded mode monitor optimisation | Planned (V3) | Level 5 Master |
...
Every Degraded Mode Monitor output MUST include:
...
6. `degraded_mode_monitor_summary` — structured domain summary (see Output Contract)
...
Raise **[DEGRADED MODE MONITOR SIMULATION REQUIRED]** when:
...
|---|---|---|---|
| 1 | Novice | 0.00–0.39 | Core degraded mode monitor analysis — analytical methods |
...
- **Regime violation**: applying degraded mode monitor method outside its stated validity range without checking boundary conditions
...
paper_section_draft:
  ...
  subsection_title: "Degraded Mode Monitor Analysis"
  content_latex: "..."
  equations_numbered: true
  notation_consistency: true

peer_review_verdict:
  target_agent_id: "degraded_mode_monitor"
  target_run_id: "..."
...
- Domain-specific standards and handbooks for degraded mode monitor
