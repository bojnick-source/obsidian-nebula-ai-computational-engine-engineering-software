# Provider Matrix Watcher — SKILL Definition

**Agent ID:** `provider_matrix_watcher`
**Domain:** Provider Matrix Watcher — LLM provider status, model availability, fallback routing
...
The Provider Matrix Watcher performs provider matrix watcher analysis, synthesising results
...
|---|---|---|
| Core provider matrix watcher analysis (analytical) | Active (MVP) | Level 1 capability |
| Provider Matrix Watcher design sizing | Active (Level 2) | Parametric methods |
| Sensitivity analysis | Planned (V1) | Level 3 |
| Numerical provider matrix watcher methods | Planned (V1) | Level 3 |
| Multi-physics provider matrix watcher coupling | Planned (V2) | Level 4 |
| Uncertainty quantification | Planned (V2) | Level 4 |
| Full-chain provider matrix watcher optimisation | Planned (V3) | Level 5 Master |
...
Every Provider Matrix Watcher output MUST include:
...
6. `provider_matrix_watcher_summary` — structured domain summary (see Output Contract)
...
confidence: 0.75
provider_matrix_watcher_summary:
  method: "analytical"
  primary_result: 0.0
  units: "[specify]"
  safety_margin: 0.0
...
Raise **[PROVIDER MATRIX WATCHER SIMULATION REQUIRED]** when:
...
|---|---|---|---|
| 1 | Novice | 0.00–0.39 | Core provider matrix watcher analysis — analytical methods |
...
- **Regime violation**: applying provider matrix watcher method outside its stated validity range without checking boundary conditions
...
paper_section_draft:
  ...
  subsection_title: "Provider Matrix Watcher Analysis"
  content_latex: "..."
  equations_numbered: true
  notation_consistency: true

peer_review_verdict:
  target_agent_id: "provider_matrix_watcher"
  target_run_id: "..."
...
- Domain-specific standards and handbooks for provider matrix watcher
